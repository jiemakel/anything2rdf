...
//id,person_id,change_timestamp,activity_id
    ...
    for (w <- wr) if (!personIdMap.contains(w(h("entity_id")))) logger.warn("Unknown entity: "+w.toSeq) else {
      ...
      val e = R(ns+"activity_"+w(h("activity_id")))
      if (!activityTypeNameMap.contains(w(h("activity_id")))) logger.warn("Unknown activity for activityTypeNameMap: "+w.toSeq)
      if (!activityPrimaryPersonNameMap.contains(w(h("activity_id")))) logger.warn("Unknown activity for activityPrimaryPersonNameMap: "+w.toSeq)
      val as = I(ns+"role_in_activity_"+w(h("id")),personNameMap(w(h("entity_id")))+" as "+roleName+" in the "+(if (activityTypeNameMap.contains(w(h("activity_id")))) activityTypeNameMap(w(h("activity_id"))) else "unknown activity type")+" of "+(if (activityPrimaryPersonNameMap.contains(w(h("activity_id")))) activityPrimaryPersonNameMap(w(h("activity_id"))) else "unknown person"),PROV.Association)
